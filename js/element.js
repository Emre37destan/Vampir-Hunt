...
_element.prototype = {
	...
	enableButton : function(id,e,txt) {
		...
		if (engine.player.isDead() && engine.dayStatus != "night") 
			this.eventMsg("Saf karanlık izin verene kadar <span>"+e+"</span> için çok zayıfsınız!");
		else {
	...
	bloodButton : function() { 
		this.goHunting = this.makeElement("button",{innerHTML:"Kan için Av", id:"bloodButton"},{width:"150px"}); 
		document.getElementById("stats").appendChild(this.goHunting);
		this.goHunting.addEventListener("click",engine.player.hunt.bind());
	},
	raidButton : function() { 
		this.goRaiding = this.makeElement("button",{innerHTML:"Altın için Baskın", id:"raidButton"},{width:"150px"}); 

...
function triggerDeath(cause) {
        var bloodloss = 20;
        addEventMsg("Şunlardan öldün: "+cause);
        addEventMsg("Ölümün sana değerli kanından "+bloodloss+" mal oldu!");
...
function healDamage(heal) {
        if (dead) {
                hp = 1;
                addEventMsg("Gecelerin güçlüsü ay sizi avlanmaya devam edecek kadar canlandırır.");
                dead = false;
        } else {
...
function raidButton() {
        goRaiding = elm("button",{innerHTML:"Altın için Baskın",id:"raidButton"},{});
...
function bloodButton() { 
        goHunting = elm("button",{innerHTML:"Kan için Av", id:"bloodButton"},{}); 
...
function enableButton(bid,bEvent,newTxt) {
        ...
        if (dead && dayStatus != "night") addEventMsg("Saf karanlık izin verene kadar "+bEvent+" için çok zayıfsınız!");
        else {
...
function hunt() {
        ...
        var bloodCollected;
        btxt.innerHTML = "avlanmayı bekle..";
        goHunting.disabled = true;
        if (dayStatus == statusCycle[0]) {
                dealDamage(10,"sunlight");
                addEventMsg("Gün ışığında avlanmak canınızı yaktı! -10 HP!");
        } else {
                bloodCollected = 1*multiplier;
                bloodCount += bloodCollected;
                blood.innerHTML = bloodCount;
                addEventMsg("Yavımız "+bloodCollected+" pint(ler) kan verdi!");
                healDamage(1);
        }
}
...
function raid() {
        ...
        if (dayStatus == statusCycle[0]) {
                ...
                addEventMsg("Gün ışığında baskın yapmak canınızı yaktı! -15 HP!");
        } else {
